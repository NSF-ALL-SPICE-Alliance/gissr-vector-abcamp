...
```{r}
# The place_id that corresponds to my target area of mississippi is 37
# genus magnolia is 82096 
```

**Challenge 2: Create a static map using the `ggplot` function of your target area and target genus/species.**

```{r}
inat_obs_df_magnolia <- get_inat_obs(taxon_id = 82096, #taxon id from URL
                            place_id = 37, #place id from URL
                            quality = "research", #specifies research grade only
                            geo = TRUE, #Specifies that we want geocoordinates
                            maxresults = 100) #Limits results

save(inat_obs_df_magnolia, file = "magnolia_mississippi_inat.Rdata")

inat_obs_sf_magnolia <-  inat_obs_df_magnolia %>% 
  select(longitude, latitude, datetime, common_name, scientific_name, image_url, user_login) %>% 
  st_as_sf(coords=c("longitude", "latitude"), crs=4326)

dim(inat_obs_sf_magnolia)

ggplot() +
  geom_point(data = inat_obs_df_magnolia, aes(x = longitude, y = latitude, color = common_name, text = common_name))

```

```{r}
mississippi_bb <- getbb("Mississippi")
mississippi_bb 
```

```{r}
# # retrieving data of streets in mississippi
# mississippi_streets <- mississippi_bb %>%
#   opq() %>%
#   add_osm_feature("highway", c("motorway", "primary", "secondary", "tertiary")) %>%
#   osmdata_sf()
# 
# # retrieving data of small streets in mississippi
# mississippi_small_streets <- mississippi_bb %>%
#   opq() %>%
#   add_osm_feature(key = "highway", value = c("residential", "living_street", "unclassified", "service", "footway")) %>%
#   osmdata_sf()

# retrieving data of coastline in mississippi
mississippi_coast <- mississippi_bb %>%
  opq() %>%
  add_osm_feature(key = "natural", value = "coastline") %>%
  osmdata_sf()

# state line? nope--county lines
# mississippi_boundaries <- mississippi_bb %>%
#   opq() %>%
#   add_osm_feature(key = "boundary", value = "administrative") %>%
#   osmdata_sf()

# retrieving data of national park in mississippi
mississippi_np <- mississippi_bb %>%
  opq() %>%
  add_osm_feature(key = "boundary", value = "national_park") %>%
  osmdata_sf()

# retrieving data of protected areas in mississippi
mississippi_protected <- mississippi_bb %>%
  opq() %>%
  add_osm_feature(key = "boundary", value = "protected_area") %>%
  osmdata_sf()
```
...
```{r}
# visualising all retrieved features over each other to form a map of mississippi
p_miss <- ggplot() +
  # geom_sf(data = mississippi_streets$osm_lines, inherit.aes = FALSE, color = "#ffbe7f", size = .4, alpha = .8) +
  # geom_sf(data = mississippi_small_streets$osm_lines, inherit.aes = FALSE, color = "#a6a6a6", size = .2, alpha = .8) +
  geom_sf(data = mississippi_coast$osm_lines, inherit.aes = FALSE, color = "black", size = .8, alpha = .5) +
  geom_sf(data = mississippi_np$osm_polygons, inherit.aes = FALSE, color = "brown", size = .2, alpha = .8) +
  geom_sf(data = mississippi_protected$osm_polygons, inherit.aes = FALSE, color = "green", size = .2, alpha = .8) +
  geom_point(data = inat_obs_df_magnolia, aes(x = longitude, y = latitude, color = common_name, text = common_name)) + # here is our iNaturalist data
  geom_sf_text(size = 1, data = mississippi_protected$osm_polygons, aes(label = name)) + #here we are adding some labels for our protected areas for context 
  coord_sf(xlim = c(-91.65501, -88.09779), ylim = c(30.14368, 34.99602), expand = TRUE) + # setting the limits of our map based on the lat/long we got from our OSM bounding box
  ggtitle("Magnolias in Mississippi", subtitle = "Based on iNaturalist Data as of October 2024 (n=1,241)") +
  theme_bw() +
  labs(
    color="Common Name", #this changes the title of our legend
    x = "Longitude",
...
#Where are the state boundaries?
# Streets were taking too long to load, so I didn't include here. That can't be why the state boundaries aren't there, right? 
...
```{r}
inat_obs_popup_sf_magnolia <- inat_obs_sf_magnolia %>% 
  mutate(popup_html = paste0("<p><b>", common_name, "</b><br/>",
                             "<i>", scientific_name, "</i></p>",
                             "<p>Observed: ", datetime, "<br/>",
                             "User: ", user_login, "</p>",
...
```{r}
htmltools::p("iNaturalist Observations of Magnolias in Mississippi",
             htmltools::br(),
             inat_obs_popup_sf_magnolia$datetime %>% 
               as.Date() %>% 
               range(na.rm = TRUE) %>% 
               paste(collapse = " to "),
             style = "font-weight:bold; font-size:110%;")
...
```{r}
leaflet(inat_obs_sf_magnolia) %>% 
  setView(lng = -89, lat = 31, zoom = 7)%>%
  addTiles() %>% 
  addCircleMarkers(data = inat_obs_popup_sf_magnolia,
                   popup = ~popup_html, 
                   radius = 5)
