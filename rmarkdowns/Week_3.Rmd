...
```{r}
available_tags("tourism") # I tried "abandoned" and it wouldn't work.
```
...
hawaiian$osm_polygons
protected$osm_polygons
```
...
After reading through the whole tutorial, come back and do: **Challenge 3: Update the bounding box in `coord_sf` with your target area.**
...
ggplot() +
  geom_sf(data = hawaiian$osm_polygons) +
  geom_sf_text(data = hawaiian$osm_polygons, aes(label = name), size = 2, hjust = 0) +
  coord_sf(xlim = c(-158.28058, -157.64863), ylim = c(21.25482, 21.71201), expand = FALSE) 
```
...
streets_oahu <- bb_oahu %>%
  opq() %>%
  add_osm_feature("highway", c("motorway", "primary", "secondary", "tertiary")) %>%
  osmdata_sf()

# retrieving data of small streets on Oahu
small_streets_oahu <- bb_oahu %>%
  opq() %>%
  add_osm_feature(key = "highway", value = c("residential", "living_street", "unclassified", "service", "footway")) %>%
  osmdata_sf()
...
ggplot() +
  geom_sf(data = streets_oahu$osm_lines, inherit.aes = FALSE, color = "#ffbe7f", size = .4, alpha = .8) +
  geom_sf(data = small_streets_oahu$osm_lines, inherit.aes = FALSE, color = "#a6a6a6", size = .2, alpha = .8) +
  geom_sf(data = coastline_oahu$osm_lines, inherit.aes = FALSE, color = "black", size = .8, alpha = .5) +
  geom_sf(data = hawaiian$osm_polygons, inherit.aes = FALSE, colour = "#0077be", fill = "#0077be", alpha = .8, size = 1) +
  ...
  geom_sf_text(data = hawaiian$osm_polygons, aes(label = name), size = 2, hjust = 0) +
  geom_sf_text(data = protected$osm_polygons, aes(label = name), size = 2, hjust = 0) +
  theme_bw() +
  labs(
    title = "Hawaiian Lands (blue) and protected lands (pink)",
    x = "Longitude",
    y = "Latitude"
  )

# another map, of higher education on Oahu
ggplot() +
  geom_sf(data = streets_oahu$osm_lines, inherit.aes = FALSE, color = "#ffbe7f", size = .4, alpha = .8) +
  geom_sf(data = small_streets_oahu$osm_lines, inherit.aes = FALSE, color = "#a6a6a6", size = .2, alpha = .8) +
  geom_sf(data = coastline_oahu$osm_lines, inherit.aes = FALSE, color = "black", size = .8, alpha = .5) +
  geom_sf(data = uni$osm_polygons, inherit.aes = FALSE, colour = "#0077be", fill = "#0077be", alpha = .8, size = 1) +
  geom_sf(data = cc$osm_polygons, inherit.aes = FALSE, colour = "red", fill = "red", alpha = .5, size = 1) +
  coord_sf(xlim = c(-158.28058, -157.64863), ylim = c(21.25482, 21.71201), expand = TRUE) +
  geom_sf_text(data = uni$osm_polygons, aes(label = name), size = 2, hjust = 0) +
  geom_sf_text(data = cc$osm_polygons, aes(label = name), size = 2, hjust = 0) +
  theme_bw() +
  labs(
    title = "Universities (blue) and Community Colleges (red) on Oʻahu",
    x = "Longitude",
    y = "Latitude"
  )
...
leaflet() %>%
  addTiles() %>%
  addPolygons(
    data = uni$osm_polygons,
    label = uni$osm_polygons$name,
    color="#0077be"
  ) %>%
  addPolygons(
    data = cc$osm_polygons,
    label = cc$osm_polygons$name,
    color="red"
  )
```
